...
| Stale or poisoned service worker              | Update registration is prompt-driven; API routes are excluded from navigation fallback and runtime API caching.                                    | Preserve versioned immutable assets and the recovery procedure when changing hosting/CDN behavior.                                                                              | Unit and installed-PWA update tests; physical device evidence remains open.                     |
