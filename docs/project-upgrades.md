...
and declares the supported 0.8.4-to-0.8.5 adjacent edge. Releases 0.4 and 0.5
are historical/EOL: they are not retroactively admitted as upgrade sources. The 0.8.5 release is
...
## Application-owned 0.8.4 to 0.8.5 checklist
...
- No dependency, lockfile, JVM, schema, or database migration is required to
  accept this edge. Do not change package versions or lockfiles solely for 0.8.5.
...
- When reviewing optional application-owned changes, include the root `AGENTS.md`
  ...
  the managed 0.8.4-to-0.8.5 edge itself.
...
For the current 0.8.4→0.8.5 edge, use the current checklist above. It is a
