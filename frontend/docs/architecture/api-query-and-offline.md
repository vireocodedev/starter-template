...
Offline support remains feature-owned: an offline adapter implements the same feature API and owns its queue, reconciliation, and storage model. `navigator.onLine` is a signal, not proof that a request can succeed. The application shell therefore combines that browser hint with an abortable same-origin readiness probe; any received HTTP response, including 4xx/5xx, proves backend reachability while a rejected or timed-out request is reported as unavailable.
