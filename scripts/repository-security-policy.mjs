...
exact(statusChecks?.required_status_checks?.map((check) => `${check.context}:${check.integration_id}`) ?? [], ["Application and deployment verification:15368", "CodeQL analysis:15368", "CodeQL:57789", "Dependency review:15368", "Secret history scan:15368"], "main ruleset required status checks");
...
const preparationEnvironment = readJson(".github/environments/template-preparation.json");
const preparationDeploymentPolicies = readJson(".github/environments/template-preparation.deployment-branch-policies.json");
const preparationLiveAssertions = readJson(".github/environments/template-preparation.live-assertions.json");
const preparationCredentials = readJson(".github/environments/template-preparation.required-credentials.json");
const preparationPublicVariables = readJson(".github/settings/template-release-automation.required-variables.json");
...
if (preparationEnvironment.wait_timer !== 0 || preparationEnvironment.prevent_self_review !== false || preparationLiveAssertions.can_admins_bypass !== false) problems.push("template-preparation environment must retain its documented no-bypass policy");
if (!Array.isArray(preparationEnvironment.reviewers) || preparationEnvironment.reviewers.length !== 0) problems.push("template-preparation must not require a recurring reviewer");
if (preparationEnvironment.deployment_branch_policy?.protected_branches !== false || preparationEnvironment.deployment_branch_policy?.custom_branch_policies !== true || JSON.stringify(preparationDeploymentPolicies) !== JSON.stringify([{ name: "main", type: "branch" }])) problems.push("template-preparation must permit exactly main workflow runs");
if (JSON.stringify(preparationCredentials.variables) !== JSON.stringify(["TEMPLATE_RELEASE_AUTOMATION_APP_ID"]) || JSON.stringify(preparationCredentials.secrets) !== JSON.stringify(["TEMPLATE_RELEASE_AUTOMATION_APP_PRIVATE_KEY"]) || typeof preparationCredentials.purpose !== "string") problems.push("template-preparation must require only the documented GitHub App ID variable and private-key secret");
if (JSON.stringify(preparationPublicVariables.variables) !== JSON.stringify(["TEMPLATE_RELEASE_AUTOMATION_APP_SLUG"]) || typeof preparationPublicVariables.purpose !== "string") problems.push("template preparation must require the documented non-secret GitHub App slug repository variable");
